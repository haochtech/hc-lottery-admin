...
public function doMobileAlldeleteCar(){
    global $_W, $_GPC;
        $res=pdo_delete('yzcj_sun_order',array('oid'=>$_GPC['id']));
        if($res){
...
public function doMobileAdoptGoods(){
     global $_W, $_GPC;
        $res=pdo_update('yzcj_sun_goods',array('status'=>2),array('gid'=>$_GPC['id']));
        if($res){
...
public function doMobileRejectGoods(){
     global $_W, $_GPC;
        $res=pdo_update('yzcj_sun_goods',array('status'=>3),array('gid'=>$_GPC['id']));
        if($res){
            message('操作成功',$this->createWebUrl('goods',array()),'success');
        }else{
            message('操作失败','','error');
...
public function doMobileDeleteGifts(){
    global $_W, $_GPC;
        ...
        $res=pdo_delete('yzcj_sun_gifts',array('id'=>$_GPC['id']));
        if($res){
...
public function doMobileAdoptGifts(){
     global $_W, $_GPC;
        $res=pdo_update('yzcj_sun_gifts',array('status'=>2),array('id'=>$_GPC['id']));
        if($res){
...
public function doMobileRejectGifts(){
     global $_W, $_GPC;
        $res=pdo_update('yzcj_sun_gifts',array('status'=>3),array('id'=>$_GPC['id']));
        if($res){
            message('操作成功',$this->createWebUrl('gifts',array()),'success');
        }else{
            message('操作失败','','error');
